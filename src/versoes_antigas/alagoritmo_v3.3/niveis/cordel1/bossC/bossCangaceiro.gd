...
var controlePadrao = false # CONTROLA SE O JOGADOR ESTÁ VISUALIZANDO OS PADRÕES OU NÃO.
...
func tocaSomTiro():
	get_node("sons/som_atirar").play()
	yield(get_tree().create_timer(0.3), "timeout")
	get_node("sons/som_atirar").stop()
...
func tocaMusicaEstagio(): # Musica -> Passou estágio
	yield(get_tree().create_timer(1), "timeout")
	$sons/passou_estagio_som.play()
	yield(get_tree().create_timer(2), "timeout")
	$sons/passou_estagio_som.stop()

func tocaMusicaPerdeu():
	yield(get_tree().create_timer(1), "timeout")
	$sons/perdeu_som.play()
	yield(get_tree().create_timer(2), "timeout")
	$sons/perdeu_som.stop()
